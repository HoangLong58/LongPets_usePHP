...
<body>
                ...
                <div class="header-with-search" style="width: 1030px;">
                    ...
                    <div class="header__search">
                        <div class="header__search-input-wrap">
                            <input type="text" id ="timkiem" class="header__search-input" placeholder="Nhập để tìm kiếm mã đơn hàng. VD: 13">
        ...
        <div class="container" style="padding:0;">
            <div class="row" style="margin-top: 15px;">
                <div class="panel panel-info" style="border-color: #ebebeb;">
                        ...
                        <div class="panel-title">
                            <div class="row">
                                <div class="col-md-6" style="color: var(--primary-color);">
                                    <h5 style="font-size: 2.6rem"><span class="glyphicon glyphicon-shopping-cart"></span> Quản lý đơn hàng</h5>
                                </div>
                                <div class="col-md-6">
                                    <a href="http://localhost/LongPets_usePHP/admin/">
                                        <button type="button" class="thanhtoan_btn" style="position:relative; left:300px;" >
                                            <span class="glyphicon glyphicon-share-alt"></span> Trở về trang quản lý
                                        </button>
                                    </a>
                                </div>
                            </div>
                    ...
                    <div id="manhanvien" value=""></div>
                    <?php
                    $sqlsoluong = "select count(madathang) as soluong from dathang;";
                    $soluong = executeResult($sqlsoluong);
                    foreach($soluong as $sl) {
                    echo '
                    <h4 style=" font-size: 2rem;position: relative; text-align: right;margin:0;top:15px; right: 20px;">Hiện có <b style="color: var(--primary-color);">'.$sl['soluong'].'</b> đơn hàng</h4>
                    ';
                    }
                    ?>
                    ...
                    <table class="table table-bordered thongke" style="margin-top: 15px; font-size: 1.6rem;">
                        ...
                        <tbody id="mytable">
                            ...
                            $sql = "select count(madathang) as number from dathang";
                            ...
                            $sql = "select * from dathang d join xaphuongthitran x on d.maxa = x.maxa join quanhuyen q on x.maquanhuyen = q.maquanhuyen join tinhthanhpho t on q.mathanhpho = t.mathanhpho join trangthaidonhang tt on d.trangthaidathang = tt.trangthaidathang order by d.ngaydathang desc limit $index , 10;";
    ...
    <script>
    $(function() {
        $(document).on('click', 'button[name = xemchitietdathang]', function() {
    ...
    <!-- SCRIPT Duyet Don Dat Hang -->
    <script>
    function DuyetDon(madathang) {
        $.post('data.php',{
            'madathang_duyet': madathang
        }, function(result){
            location.reload();
        })
    }
    ...
    function HuyDon(madathang) {
        if(confirm("Bạn muốn từ chối đơn đặt hàng này?") == true){
            $.post('data.php',{
    ...
    <!-- Tim kiem ma don hang -->
    <script>
    $(document).ready(function() {
    $("#timkiem").on("keyup", function() {
        var timkiem = $("#timkiem").val().toLowerCase();
        $.post("data.php", { send_timkiemdh: timkiem }, function (result) {
        $('#mytable').html(result);
        })
    });
    });
